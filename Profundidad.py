...
pila = [nodo_inicial]
visitados = set()  # Guardamos las posiciones visitadas
...
def expandir_nodo():
    while pila:  
        ...
        if verificar_si_es_paquete(nodo_actual.estado, posicion_paquetes):
            print(f"Encontrado paquete en {nodo_actual.estado}")
            return  
        ...
        for movimiento, operador in zip(movimientos, ["derecha", "izquierda", "arriba", "abajo"]):
            if movimiento is not None and movimiento not in visitados: 
                ...
                print(f'Estamos en {nodo_actual.estado}, nos movemos {operador} a {movimiento}')
